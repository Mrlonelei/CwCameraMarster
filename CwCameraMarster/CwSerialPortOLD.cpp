...
#include <assert.h>
CString				CwSerialPortCamera::m_strRecv = "";
int m_nComArray[20];
//
// Constructor
//
CwSerialPortCamera::CwSerialPortCamera()///构造函数
{
	m_hComm = NULL;
...
//
CwSerialPortCamera::~CwSerialPortCamera()///析构函数
{
	do
	{
...
//
BOOL CwSerialPortCamera::InitPort(CWnd* pPortOwner,	// the owner (CWnd) of the port (receives message)
...
BOOL CwSerialPortCamera::InitPort(CWnd* pPortOwner,	// the owner (CWnd) of the port (receives message)
						   char*  portN,		// portnumber (1..MaxSerialPortNum)
						   UINT  baud,			// baudrate
						   char  parity,		// parity 
						   UINT  databits,		// databits 
						   UINT  stopbits,		// stopbits 
						   ...
						   UINT  writebuffersize)	// size to the writebuffer
{
	...
	*/
	...
	// prepare port strings
	///串口参数
	sprintf(szPort, "\\\\.\\%s", portN);///可以显示COM10以上端口//add by itas109 2014-01-09 \\\\.\\ //
	sprintf(szBaud, "baud=%d parity=%c data=%d stop=%d", baud, parity, databits, stopbits);
...
//
UINT CwSerialPortCamera::CommThread(LPVOID pParam)
{
	// Cast the void pointer passed to the thread back to
	// a pointer of CSerialPort class
	///CSerialPort类的指针
	CwSerialPortCamera *port = (CwSerialPortCamera*)pParam;
	CString strout;
	...
	strout.Format("write buff size  is %d", port->GetWriteBufferSize());
	TRACE(strout);
	...
	{ 
		...
		*/
		...
		else	///WaitCommEvent()能正确返回
		{
			...
			strout.Format("comstat.cbInQue  %d", comstat.cbInQue);
			TRACE(strout);
...
///开启监视线程
//
BOOL CwSerialPortCamera::StartMonitoring()
{
	if (!(m_Thread = AfxBeginThread(CommThread, this,0,0,0,0)))
		return FALSE;
...
///复位监视线程
//
BOOL CwSerialPortCamera::RestartMonitoring()
{
...
///挂起监视线程
//
BOOL CwSerialPortCamera::StopMonitoring()
{
...
///如果有错误，给出提示
//
void CwSerialPortCamera::ProcessErrorMessage(char* ErrorText)
{
...
///写数据
//
void CwSerialPortCamera::WriteChar(CwSerialPortCamera* port)
{
	BOOL bWrite = TRUE;
	BOOL bResult = TRUE;
...
//							strlen((char*)port->m_szWriteBuffer),	// Length of message to send
		...
		strout.Format("write char result %d", bResult);
		TRACE(strout);
	...
	strout.Format("write byte:%d", BytesSent);
	TRACE(strout);
...
///读数据
//
void CwSerialPortCamera::ReceiveChar(CwSerialPortCamera* port, COMSTAT comstat)
{
	BOOL  bRead = TRUE; 
	BOOL  bResult = TRUE;
	DWORD dwError = 0;
	...
	{ 
		...
		///所有字符均被读出，中断循环
		strout.Format("receivechar comstat.cbInQue: %d\n", comstat.cbInQue);
		TRACE(strout);
		if (comstat.cbInQue == 0)
		{
...
//
void CwSerialPortCamera::WriteToPort(char* string)
{		
	assert(m_hComm != 0);
...
void CwSerialPortCamera::WriteToPort(char* string,int n)
{		
	assert(m_hComm != 0);
...
void CwSerialPortCamera::WriteToPort(LPCTSTR string,int n)
{		
	assert(m_hComm != 0);
...
void CwSerialPortCamera::WriteToPort(LPCTSTR string)
{		
	assert(m_hComm != 0);
...
void CwSerialPortCamera::WriteToPort(BYTE* Buffer, int n)
{
	assert(m_hComm != 0);
	memset(m_szWriteBuffer, 0, sizeof(m_szWriteBuffer));
	int i;
...
//
DWORD CwSerialPortCamera::GetCommEvents()
{
...
//
DWORD CwSerialPortCamera::GetWriteBufferSize()
{
	return m_nWriteBufferSize;
}


void CwSerialPortCamera::ClosePort()
{
    if(m_bIsSuspened)
	{
		RestartMonitoring();
...
void CwSerialPortCamera::SendData(LPCTSTR lpszData, const int nLength)
{
	assert(m_hComm != 0);
	memset(m_szWriteBuffer, 0, 1024*sizeof(char));
...
BOOL CwSerialPortCamera::RecvData(LPTSTR lpszData, const int nSize)
{
    //
    //接收数据
    //
...
//     }
//     
	int Minsize = nSize < m_strRecv.GetLength() ? nSize : m_strRecv.GetLength();
	for (int size = 0; size < Minsize; size++)
	{
		lpszData[size] = m_strRecv.GetAt(size);
	}
    return TRUE;
}

void CwSerialPortCamera::ClearReadBuffer()
{
	m_strRecv = "";
}

COMMTIMEOUTS CwSerialPortCamera:: GetCommTimeOuts()
{
	return m_CommTimeouts;
}

BOOL CwSerialPortCamera::SetCommTimeOuts(COMMTIMEOUTS *lpTimeOuts)
{
...
//
void CwSerialPortCamera::QueryKey(HKEY hKey) 
{ 
	...
	TCHAR    achClass[MAX_CHAR] = TEXT("");  // buffer for class name 
	DWORD    cchClassName = MAX_CHAR;  // size of class string 
...
void CwSerialPortCamera::Hkey2ComboBox(CComboBox& m_PortNO)
{
	HKEY hTestKey;
	bool Flag = FALSE;
